...
public sealed class LmaxDemoDuplicateOrderRetirementTests
{
    [Fact]
    ...
    public void AnySendIntentOrCompletionPreventsRetirement(string lifecycle)
    {
        using var f = new Fixture(lifecycle);
        Assert.Throws<InvalidOperationException>(() => f.Prepare());
        Assert.False(File.Exists(LmaxDemoDuplicateOrderRetirement.CertificatePath(f.Path)));
    }
    ...
    {
        using var f = new Fixture();
        Assert.Throws<InvalidOperationException>(() => LmaxDemoDuplicateOrderRetirement.Prepare(f.Path,
            LmaxDemoDuplicateOrderRetirement.HashFile(f.Path), Fixture.Approval, f.Observation, f.Database, f.Now));
    }
    ...
    public void DatabaseDisagreementPreventsRetirement(string condition)
    {
        ...
        {
            "status" => f.Database with { ChildStatus = "PendingNew" },
            "deadline" => f.Database with { DispositionAtUtc = DateTimeOffset.Parse("2026-09-18T16:00:00Z") },
            "child" => f.Database with { ChildOrderId = Guid.NewGuid().ToString() },
            "fill" => f.Database with { NewFills = 1 },
            "working" => f.Database with { OpenChildOrders = 1 },
            "position" => f.Database with { NonZeroPositions = 1 },
            "parent" => f.Database with { NewParentOrders = 2 },
            "target" => f.Database with { FailedRunTargets = 3 },
            "processed" => f.Database with { FailedRunProcessed = true },
            _ => f.Database with { FullPortfolioWeightCount = 13 }
        };
    ...
    [Theory]
    [InlineData("stale")]
    [InlineData("before_fault")]
    [InlineData("working")]
    [InlineData("not_flat")]
    [InlineData("wrong_account")]
    public void ObservationMustBeFreshAndAfterTheIncident(string condition)
    {
        using var f = new Fixture();
        var observation = condition switch
        {
            "stale" => f.Observation with { ObservedAtUtc = f.Now.AddMinutes(-20) },
            "before_fault" => f.Observation with { ObservedAtUtc = f.StartAt },
    ...
    public void ApprovalAndExactJournalHashAreRequired()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => LmaxDemoDuplicateOrderRetirement.Prepare(f.Path,
            LmaxDemoDuplicateOrderRetirement.HashFile(f.Path), "", f.Observation, f.Database, f.Now, true));
    }
    ...
    public void CertificateCannotBeOverwrittenOrForgedForAnotherJournal()
    {
        ...
        var path = LmaxDemoDuplicateOrderRetirement.CertificatePath(f.Path);
        var bytes = File.ReadAllBytes(path);
        using (f.Lease()) Assert.Throws<IOException>(() => LmaxDemoDuplicateOrderRetirement.WriteUnderOwnerLease(f.Path, certificate, f.Now, true));
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal DateTimeOffset StartAt { get; } = new(2026, 9, 18, 16, 1, 0, TimeSpan.Zero);
        ...
        internal LmaxDemoRetirementObservation Observation => new(Start.SessionId, Start.AccountId, StartAt.AddSeconds(3), true, true, true, Approval);
        internal LmaxDemoDuplicateOrderDatabaseEvidence Database => new(Now, "LMAX_DEMO_LOCAL", LmaxDemoDuplicateOrderRetirement.FailedModelRunId,
            false, 14, 2, 2, 2, 1, 1, 0, 0, 0, 0,
            "f63dba82-e46a-4a38-b773-92edd01dadfa", "4f900cc0-ace7-42f7-be9a-ebb03ea8f95f",
            "Expired", "Expired", "LOCAL_PRE_SEND_DEADLINE_EXPIRED", Now);
        ...
        internal Fixture(string? lifecycle = null, string fault = "COORDINATOR_RECONCILIATION_REQUIRED")
        {
            ...
            var cycleId = Guid.Parse(LmaxDemoDuplicateOrderRetirement.FailedModelRunId).ToString("N");

            if (lifecycle is not null)
            {
                owner.Session.BeginCycle(cycleId, new Dictionary<string, decimal> { ["EURUSD"] = -20000m }, StartAt);
                if (lifecycle is "send" or "completed")
                {
                    owner.Session.RecordSendIntent(new(cycleId, "parent", "child", "D", null, "EURUSD", "4001", "SELL", 2m,
            ...
            // A real declaration may precede the final heartbeat/stop. The
            // unchanged no-send journal bridges that interval without retiming it.
            owner.Session.RecordInboundControl(2, "0", StartAt.AddMilliseconds(3500));
        }
        internal FileStream Lease() => new(System.IO.Path.Combine(Root, "owner.lock"), FileMode.Open, FileAccess.ReadWrite, FileShare.None);
        internal LmaxDemoDuplicateOrderRetirementCertificate Prepare(LmaxDemoRetirementObservation? observation = null,
            LmaxDemoDuplicateOrderDatabaseEvidence? database = null) => LmaxDemoDuplicateOrderRetirement.Prepare(Path,
                LmaxDemoDuplicateOrderRetirement.HashFile(Path), Approval, observation ?? Observation, database ?? Database, Now, true);
        internal LmaxDemoDuplicateOrderRetirementCertificate Retire()
        {
            using var lease = Lease();
            var certificate = Prepare();
            LmaxDemoDuplicateOrderRetirement.WriteUnderOwnerLease(Path, certificate, Now, true);
            return certificate;
        }
