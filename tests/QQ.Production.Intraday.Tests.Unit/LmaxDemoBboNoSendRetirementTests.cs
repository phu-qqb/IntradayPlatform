...
public sealed class LmaxDemoBboNoSendRetirementTests
{
    [Fact]
    public void ValidRetirementRetainsTheFaultedJournalAndPermitsOneNewOwner()
    {
        ...
        Assert.Equal("PARENT_RECONCILIATION_REQUIRED", LmaxDemoControlledSession.Inspect(prior).BlockingReason);
    ...
    public void AnySendIntentOrCompletionPreventsRetirement(string lifecycle)
    {
        ...
        Assert.False(File.Exists(LmaxDemoBboNoSendRetirement.CertificatePath(f.Path)));
    }
    ...
    {
        using var f = new Fixture();
        Assert.Throws<InvalidOperationException>(() => LmaxDemoBboNoSendRetirement.Prepare(f.Path,
            LmaxDemoBboNoSendRetirement.HashFile(f.Path), Fixture.Approval, f.Observation, f.Database, f.Now));
    }
    ...
    public void DatabaseDisagreementPreventsRetirement(string condition)
    {
        ...
        var database = condition switch
        {
            ...
            "deadline" => f.Database with { DispositionAtUtc = DateTimeOffset.Parse("2026-09-17T16:30:00Z") },
            ...
            "target" => f.Database with { FailedRunTargets = 2 },
    ...
    public void ApprovalAndExactJournalHashAreRequired()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => LmaxDemoBboNoSendRetirement.Prepare(f.Path,
            LmaxDemoBboNoSendRetirement.HashFile(f.Path), "", f.Observation, f.Database, f.Now, true));
    }
    ...
    public void CertificateCannotBeOverwrittenOrForgedForAnotherJournal()
    {
        ...
        var path = LmaxDemoBboNoSendRetirement.CertificatePath(f.Path);
        var bytes = File.ReadAllBytes(path);
        using (f.Lease()) Assert.Throws<IOException>(() => LmaxDemoBboNoSendRetirement.WriteUnderOwnerLease(f.Path, certificate, f.Now, true));
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal DateTimeOffset StartAt { get; } = new(2026, 9, 17, 16, 31, 0, TimeSpan.Zero);
        ...
        internal LmaxDemoUnsentParentDatabaseEvidence Database => new(Now, "LMAX_DEMO_LOCAL", LmaxDemoBboNoSendRetirement.FailedModelRunId,
            false, 39, 1, 1, 1, 1, 1, 0, 0, 0, 0,
            "20c4d977-05e4-46b5-8dc3-34c887ed26a4", "dd4194b6-01a0-4561-9708-a801b3973bd6",
            "Expired", "Expired", "LOCAL_PRE_SEND_DEADLINE_EXPIRED", Now);

        internal Fixture(string? lifecycle = null, string fault = "PARENT_RECONCILIATION_REQUIRED")
        {
            ...
            var cycleId = Guid.Parse(LmaxDemoBboNoSendRetirement.FailedModelRunId).ToString("N");
            owner.Session.BeginCycle(cycleId, new Dictionary<string, decimal> { ["EURUSD"] = -1000m }, StartAt);
            if (lifecycle is not null)
            {
                if (lifecycle is "send" or "completed")
                {
                    owner.Session.RecordSendIntent(new(cycleId, "parent", "child", "D", null, "EURUSD", "4001", "SELL", .1m,
        ...
        internal LmaxDemoUnsentParentRetirementCertificate Prepare(LmaxDemoRetirementObservation? observation = null,
            LmaxDemoUnsentParentDatabaseEvidence? database = null) => LmaxDemoBboNoSendRetirement.Prepare(Path,
                LmaxDemoBboNoSendRetirement.HashFile(Path), Approval, observation ?? Observation, database ?? Database, Now, true);
        internal LmaxDemoUnsentParentRetirementCertificate Retire()
        {
            using var lease = Lease();
            var certificate = Prepare();
            LmaxDemoBboNoSendRetirement.WriteUnderOwnerLease(Path, certificate, Now, true);
            return certificate;
        }
