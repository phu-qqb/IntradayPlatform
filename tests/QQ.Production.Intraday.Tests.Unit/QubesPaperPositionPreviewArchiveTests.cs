...
public sealed class QubesPaperPositionPreviewArchiveTests
{
    ...
    [Fact]
    public async Task R020_paper_position_preview_can_be_archived_no_externally()
    {
        var result = await ArchiveAsync(await CreateContextAsync());

        Assert.True(result.Persisted);
        Assert.Equal(PaperPositionPreviewArchiveStatus.ArchivedNoExternal, result.ArchiveRecord.ArchiveStatus);
    }
    ...
    public async Task Archive_preserves_paper_position_preview_id()
    {
        ...
        Assert.Equal(context.Review.PaperPositionPreview!.PaperPositionPreviewId, result.ArchiveRecord.PaperPositionPreviewId);
    }
    ...
    public async Task Archive_preserves_paper_simulation_result_id()
    {
        ...
        Assert.Equal(context.Review.PaperPositionPreview!.PaperSimulationResultId, result.ArchiveRecord.PaperSimulationResultId);
    }
    ...
    public async Task Archive_preserves_paper_simulation_plan_id()
    {
        ...
        Assert.Equal(context.Review.PaperPositionPreview!.PaperSimulationPlanId, result.ArchiveRecord.PaperSimulationPlanId);
    }
    ...
    public async Task Archive_preserves_paper_execution_plan_id()
    {
        ...
        Assert.Equal(context.Review.PaperPositionPreview!.PaperExecutionPlanId, result.ArchiveRecord.PaperExecutionPlanId);
    }
    ...
    public async Task Archive_preserves_cycle_run_id_and_qubes_run_id()
    {
        ...
        Assert.Equal("cycle-r021-sample", result.ArchiveRecord.CycleRunId);
        Assert.Equal("qubes-r021-sample", result.ArchiveRecord.QubesRunId);
    }
    ...
    public async Task Archive_preserves_operator_decision_id()
    {
        ...
        Assert.Equal(context.Review.PaperPositionPreview!.OperatorDecisionId, result.ArchiveRecord.OperatorDecisionId);
    }
    ...
    public async Task Preview_lines_include_audusd_delta()
    {
        ...
        Assert.Contains(result.ArchiveRecord.Lines, x => x.NormalizedSymbol == "AUDUSD" && x.SimulatedPositionDelta == 131000m && x.QuantityCurrency == "AUD");
    }
    ...
    public async Task Preview_lines_include_eurusd_delta()
    {
        ...
        Assert.Contains(result.ArchiveRecord.Lines, x => x.NormalizedSymbol == "EURUSD" && x.SimulatedPositionDelta == 124000m && x.QuantityCurrency == "EUR");
    }

    [Fact]
    public async Task Preview_lines_include_gbpusd_sell_delta()
    {
        var result = await ArchiveAsync(await CreateContextAsync());

        Assert.Contains(result.ArchiveRecord.Lines, x => x.NormalizedSymbol == "GBPUSD" && x.SimulatedPositionDelta == -368000m && x.QuantityCurrency == "GBP");
    }

    [Fact]
    public async Task Operator_report_includes_paper_only_and_simulated_only_disclaimer()
    {
        var result = await ArchiveAsync(await CreateContextAsync());

        Assert.True(result.OperatorReport.IncludesPaperPositionPreviewOnlyDisclaimer);
        Assert.True(result.OperatorReport.IncludesSimulatedOnlyDisclaimer);
        Assert.Contains("Paper position preview only", result.OperatorReport.Markdown);
        Assert.Contains("Simulated-only", result.OperatorReport.Markdown);
    }
    ...
    [Fact]
    public async Task Operator_report_includes_no_order_fill_report_route_or_submission_disclaimer()
    {
        var result = await ArchiveAsync(await CreateContextAsync());
    ...
    [Fact]
    public async Task Duplicate_preview_archive_handling_is_idempotent()
    {
        var context = await CreateContextAsync();
        var repository = new InMemoryPaperPositionPreviewArchiveRepository();
        var service = new PaperPositionPreviewArchiveService(repository, new FixedClock(ProducedAt));

        var first = await service.ArchiveAsync(context.Review.PaperPositionPreview!, CancellationToken.None);
        var second = await service.ArchiveAsync(context.Review.PaperPositionPreview!, CancellationToken.None);
        ...
        Assert.Equal(PaperPositionPreviewArchiveStatus.DuplicateReturned, second.ArchiveRecord.ArchiveStatus);
    }
    ...
    public async Task No_live_position_state_is_mutated()
    {
        ...
        Assert.True(result.ArchiveRecord.NoLivePositionMutation);
    ...
    public async Task No_broker_position_state_is_mutated()
    {
        ...
        Assert.True(result.ArchiveRecord.NoBrokerPositionMutation);
    ...
    public async Task No_trading_state_is_mutated()
    {
        ...
        Assert.True(result.ArchiveRecord.NoTradingStateMutation);
    ...
    public async Task No_order_submission_path_is_introduced()
    {
        ...
        Assert.False(result.ArchiveRecord.BrokerRouteCreated);
    }

    [Fact]
    public void Archive_source_introduces_no_broker_socket_tls_fix_or_marketdata_runtime_action()
    {
        var source = File.ReadAllText(Path.Combine(RepoRoot(), "src/QQ.Production.Intraday.Application/QubesPaperPositionPreviewArchive.cs"));
    ...
    private static async Task<PaperPositionPreviewArchiveResult> ArchiveAsync(TestContext context)
        => await new PaperPositionPreviewArchiveService(
                new InMemoryPaperPositionPreviewArchiveRepository(),
                new FixedClock(ProducedAt))
            .ArchiveAsync(context.Review.PaperPositionPreview!, CancellationToken.None);
    ...
    private static async Task<TestContext> CreateContextAsync()
    {
        ...
        var cycle = await services.Cycle.RunOneCycleAsync(new QubesIntradayCycleFixtureRequest(
            "cycle-r021-sample",
            new QubesRunId("qubes-r021-sample"),
            ProducedAt,
            EffectiveAt,
            "QQ_MASTER",
            "IntradayFxModel",
        ...
        var review = await services.OperatorReview.ReviewAsync(
            ...
            new IntradayCycleOperatorDecisionRequest(
                ...
                "No-external paper position preview archive.",
        ...
        return new TestContext(preview);
    }
    ...
    private static TestServices CreateServices()
    {
        ...
        var normalized = new QubesFxWeightsFixtureIngestionService().ParseNormalizeAndMap(new QubesFxWeightsIngestionRequest(
            new QubesRunId("qubes-r021-reference"),
            ProducedAt,
            EffectiveAt,
            15,
            "QQ_MASTER",
    ...
    private sealed record TestContext(PaperSimulationResultOperatorReviewResult Review);
