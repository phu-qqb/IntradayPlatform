...
public sealed class LmaxDemoUnsentParentRetirementTests
{
    [Fact]
    ...
    public void AnySendIntentOrCompletionPreventsRetirement(string lifecycle)
    {
        ...
        Assert.False(File.Exists(LmaxDemoUnsentParentRetirement.CertificatePath(f.Path)));
    }
    ...
    {
        using var f = new Fixture();
        Assert.Throws<InvalidOperationException>(() => LmaxDemoUnsentParentRetirement.Prepare(f.Path,
            LmaxDemoUnsentParentRetirement.HashFile(f.Path), Fixture.Approval, f.Observation, f.Database, f.Now));
    }
    ...
    public void DatabaseDisagreementPreventsRetirement(string condition)
    {
        ...
        var database = condition switch
        {
            ...
            "deadline" => f.Database with { DispositionAtUtc = DateTimeOffset.Parse("2026-09-17T15:30:00Z") },
    ...
    public void ApprovalAndExactJournalHashAreRequired()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => LmaxDemoUnsentParentRetirement.Prepare(f.Path,
            LmaxDemoUnsentParentRetirement.HashFile(f.Path), "", f.Observation, f.Database, f.Now, true));
    }
    ...
    public void CertificateCannotBeOverwrittenOrForgedForAnotherJournal()
    {
        ...
        var path = LmaxDemoUnsentParentRetirement.CertificatePath(f.Path);
        var bytes = File.ReadAllBytes(path);
        using (f.Lease()) Assert.Throws<IOException>(() => LmaxDemoUnsentParentRetirement.WriteUnderOwnerLease(f.Path, certificate, f.Now, true));
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal DateTimeOffset StartAt { get; } = new(2026, 9, 17, 15, 31, 0, TimeSpan.Zero);
        ...
        internal LmaxDemoUnsentParentDatabaseEvidence Database => new(Now, "LMAX_DEMO_LOCAL", LmaxDemoUnsentParentRetirement.FailedModelRunId,
            false, 39, 1, 1, 1, 1, 1, 0, 0, 0, 0,
            "ffaf6e28-a6ef-46fc-988f-c93675725534", "97abae50-3e10-4bcc-a846-f830ca0c2faa",
        ...
        internal Fixture(string? lifecycle = null, string fault = "COORDINATOR_RECONCILIATION_REQUIRED")
        {
            ...
            var cycleId = Guid.Parse(LmaxDemoUnsentParentRetirement.FailedModelRunId).ToString("N");
        ...
        internal LmaxDemoUnsentParentRetirementCertificate Prepare(LmaxDemoRetirementObservation? observation = null,
            LmaxDemoUnsentParentDatabaseEvidence? database = null) => LmaxDemoUnsentParentRetirement.Prepare(Path,
                LmaxDemoUnsentParentRetirement.HashFile(Path), Approval, observation ?? Observation, database ?? Database, Now, true);
        internal LmaxDemoUnsentParentRetirementCertificate Retire()
        {
            using var lease = Lease();
            var certificate = Prepare();
            LmaxDemoUnsentParentRetirement.WriteUnderOwnerLease(Path, certificate, Now, true);
            return certificate;
        }
