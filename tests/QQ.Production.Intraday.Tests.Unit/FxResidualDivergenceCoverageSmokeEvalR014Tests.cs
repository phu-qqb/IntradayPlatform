...
public sealed class FxResidualDivergenceCoverageSmokeEvalR014Tests
{
    private static readonly DateTimeOffset ValidationTime = new(2026, 05, 28, 16, 00, 00, TimeSpan.Zero);
    private static readonly DateTimeOffset LockedStartUtc = new(2025, 07, 01, 00, 00, 00, TimeSpan.Zero);
    private static readonly DateTimeOffset LockedEndUtc = new(2025, 07, 03, 23, 59, 59, TimeSpan.Zero);
    ...
    [Fact]
    public void Approved_r013_manifest_runs_locked_coverage_smoke_evaluation()
    {
        var root = FindRepoRoot();
        var artifactsPath = ArtifactsPath(root);
        Directory.CreateDirectory(artifactsPath);

        var parameterLockPath = Path.Combine(artifactsPath, "PARAMETER_LOCK_R014.md");
        Assert.True(File.Exists(parameterLockPath), "PARAMETER_LOCK_R014.md must exist before approved R012 quote rows may be opened.");
        ...
        Assert.Equal(559644, quotes.Length);
        ...
        WriteRunSummary(artifactsPath, new
        {
            ...
            recommendedNextPackage = RecommendNextPackage(pipeline.Summary.AcceptedSignalsCount, pipeline.Summary.Lines.Count, residualDiagnostics.MaxAbsoluteZScore)
        });
    }

    [Fact]
    public void Synthetic_r014_availability_and_quote_boundaries_are_enforced()
    {
    ...
    [Fact]
    public void Synthetic_r014_diagnostic_evaluation_starts_at_next_grid_step()
    {
    ...
    [Fact]
    public void Synthetic_r014_future_rows_do_not_change_prior_pipeline_or_contained_evaluation()
    {
    ...
    [Fact]
    public void R014_code_does_not_bind_execution_sizing_or_production_outputs()
    {
        var root = FindRepoRoot();
        var source = File.ReadAllText(Path.Combine(root, "tests", "QQ.Production.Intraday.Tests.Unit", "FxResidualDivergenceCoverageSmokeEvalR014Tests.cs"));
    ...
    {
        Assert.True(File.Exists(manifestPath), "The R013 approved manifest must exist.");
    ...
    private static string RecommendNextPackage(int acceptedSignals, int evaluatedSignals, double? maxAbsoluteZScore)
    {
        if (acceptedSignals > 0 && evaluatedSignals > 0)
        {
            return "NEXT_INTRADAY_FX_RESIDUAL_DIVERGENCE_PREREGISTERED_EVAL_R015";
        }

        if (acceptedSignals == 0 && maxAbsoluteZScore is >= 1.75)
        {
            return "NEXT_INTRADAY_FX_RESIDUAL_DIVERGENCE_PREREGISTERED_SENSITIVITY_R015";
        }

        return "NEXT_INTRADAY_FX_RESIDUAL_DIVERGENCE_MODEL_DIAGNOSTICS_R015";
    }
    ...
    private static FxBboQuoteResearch Quote(string symbol, DateTimeOffset timestampUtc, decimal midpoint, DateTimeOffset availableAtUtc)
    {
        ...
        return new(symbol, timestampUtc, midpoint - halfSpread, midpoint + halfSpread, AvailableAtUtc: availableAtUtc, Source: "SyntheticR014");
    }
    ...
    private static void WriteRunSummary(string artifactsPath, object summary)
    {
        ...
        File.WriteAllText(Path.Combine(artifactsPath, "local-coverage-smoke-eval-r014-run-summary.json"), json);
    }
    ...
    private static string ApprovedManifestPath(string root)
        => Path.Combine(
            ...
            "artifacts",
            "readiness",
            "intraday-polygon-fx-tick-coverage-manifest-approval-r013",
            "POLYGON_FX_TICK_COVERAGE_BACKFILL_R012_APPROVED.local.json");

    private static string ArtifactsPath(string root)
        => Path.Combine(root, "artifacts", "readiness", "intraday-fx-residual-divergence-coverage-smoke-eval-r014");
