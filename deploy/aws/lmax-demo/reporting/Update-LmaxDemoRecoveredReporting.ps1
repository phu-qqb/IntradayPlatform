...
$root='C:\deploy\IntradayPlatform\operator\lmax-demo-orchestration\daily-eod-recovery-20260918'
$oldRoot='C:\deploy\IntradayPlatform\operator\lmax-demo-orchestration\daily-eod-portal-20260918'
...
$backup='D:\data\lmax-demo-orchestration\reconciliation-20260918\daily-eod-task-before-recovery-'+[Guid]::NewGuid().ToString()+'.xml'
...
$task.Description='LMAX Demo EOD: qualified report acquisition, import, reconciliation and provisional recap with explicit recovered-fill provenance. No trading or email. Requires Administrator logged on.'
...
$result=[ordered]@{schema='lmax_demo_recovered_reporting_task_update_v1';observed_utc=[DateTime]::UtcNow.ToString('o');task_name=$name;backup_path=$backup;qualification_receipt=$full;qualification_sha256=(Get-FileHash -LiteralPath $full -Algorithm SHA256).Hash.ToLowerInvariant();runtime=$root;arguments=$after.Actions[0].Arguments;state=[string]$after.State;next_run_utc=$info.NextRunTime.ToUniversalTime().ToString('o');principal_unchanged=$true;settings_unchanged=$true;triggers_unchanged=$true;trading_started=$false;email_sent=$false}
$output='D:\data\lmax-demo-orchestration\reconciliation-20260918\daily-eod-task-recovery-update-'+[Guid]::NewGuid().ToString()+'.json'
