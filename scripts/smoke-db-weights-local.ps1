...
function Get-ApiId {
  ...
  if ($Value -is [string]) { if ([string]::IsNullOrWhiteSpace($Value)) { return $null }; return $Value }
  if ($Value -is [Guid]) { return $Value.ToString() }
  $valueProperty = $Value.PSObject.Properties["value"]
  if ($null -ne $valueProperty -and $null -ne $valueProperty.Value) { return Get-ApiId $valueProperty.Value }
  $idProperty = $Value.PSObject.Properties["id"]
  if ($null -ne $idProperty -and $null -ne $idProperty.Value) { return Get-ApiId $idProperty.Value }
  $text = [string]$Value
...
Invoke-LocalApi Get "/health"
Invoke-LocalApi Get "/admin/reference-data/integrity"
...
Invoke-LocalApi Post "/market-data/build-bars" @{
  venueName = "LMAX"
...
Invoke-LocalApi Get "/model-weight-batches"
Invoke-LocalApi Get "/model-weight-batches/$batchId/rows"
Invoke-LocalApi Post "/model-weight-batches/$batchId/validate"
...
if ($null -ne $modelRunId) {
  Invoke-LocalApi Post "/model-runs/$modelRunId/process"
}
