...
dotnet run --project $project --no-build --no-restore -- "account-api-positions-smoke" "--environment=$EnvironmentName" "--allow-external-connections=$($AllowExternalConnections.IsPresent)" "--account-api-auth-mode=$AuthMode" "--show-response-excerpt=$($ShowResponseExcerpt.IsPresent)"
