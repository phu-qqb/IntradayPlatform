param(
    ...
    [string]$Reason = "Phase 7C explicit manual GBPUSD MarketDataOnly evidence replay"
)
...
if ([string]$evidence.evidenceMode -ne "MarketDataOnly" -or [string]$evidence.instrument -ne "GBPUSD" -or [string]$evidence.securityId -ne "4002") { throw "Phase 7C requires GBPUSD MarketDataOnly evidence preview." }
if (@($evidence.executionReports).Count -ne 0 -or @($evidence.orderStatuses).Count -ne 0 -or @($evidence.tradeCaptureReports).Count -ne 0 -or @($evidence.protocolRejects).Count -ne 0) { throw "GBPUSD MarketDataOnly replay requires empty non-market-data arrays." }
...
$outPath = Join-Path $outDir "phase7c-gbpusd-market-hours-evidence-replay.json"
