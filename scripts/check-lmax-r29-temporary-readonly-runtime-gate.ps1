...
function Assert-In($Actual, [object[]]$Allowed, [string]$Message) {
    if ($Allowed -notcontains $Actual) { throw "$Message. Expected one of '$($Allowed -join "', '")' but got '$Actual'." }
}

function Assert-True($Actual, [string]$Message) {
    if ($Actual -ne $true) { throw "$Message. Expected true but got '$Actual'." }
}

function Assert-False($Actual, [string]$Message) {
    if ($Actual -ne $false) { throw "$Message. Expected false but got '$Actual'." }
}
...
function Assert-NoSensitiveContent([string]$Path) {
    ...
    foreach ($pattern in $patterns) {
        if ($text -match $pattern) { throw "Sensitive-content marker found in $Path" }
    }
}
...
function Assert-TextContains([string]$Text, [string]$Needle, [string]$Message) {
    if ($Text -notmatch [regex]::Escape($Needle)) { throw "$Message. Missing '$Needle'." }
}

function Assert-TextNotContainsPattern([string]$Text, [string]$Pattern, [string]$Message) {
    if ($Text -match $Pattern) { throw "$Message. Matched '$Pattern'." }
}
...
$requiredPrior = @(
    ...
    "phase-lmax-r28-gate-validation.json"
)

$requiredR29 = @(
    "phase-lmax-r29-operator-approval-record.json",
    "phase-lmax-r29-preflight-gate.json",
    "phase-lmax-r29-final-provider-stack-validation.json",
    "phase-lmax-r29-temporary-runtime-activation-record.json",
    "phase-lmax-r29-approved-instrument-status-record.json",
    "phase-lmax-r29-sanitized-runtime-boundary-evidence.json",
    "phase-lmax-r29-forbidden-action-validation.json",
    "phase-lmax-r29-shutdown-revert-record.json",
    "phase-lmax-r29-post-attempt-non-mutation-validation.json",
    "phase-lmax-r29-rail-isolation-validation.json",
    "phase-lmax-r29-decision-gate.json",
    "phase-lmax-r29-temporary-readonly-runtime-report.md",
    "phase-lmax-r29-operator-note.md"
)

Write-Host "LMAX-R29 Temporary Read-Only Runtime Gate Validator"
Write-Host "This validator verifies R29 artifacts and confirms no forbidden action occurred."

Assert-RequiredArtifacts $readiness $requiredPrior
Assert-RequiredArtifacts $readiness $requiredR29
...
$r28Gate = Read-Json (Join-Path $readiness "phase-lmax-r28-decision-gate.json")
$approval = Read-Json (Join-Path $readiness "phase-lmax-r29-operator-approval-record.json")
$preflight = Read-Json (Join-Path $readiness "phase-lmax-r29-preflight-gate.json")
$stack = Read-Json (Join-Path $readiness "phase-lmax-r29-final-provider-stack-validation.json")
$activation = Read-Json (Join-Path $readiness "phase-lmax-r29-temporary-runtime-activation-record.json")
$instrumentStatus = Read-Json (Join-Path $readiness "phase-lmax-r29-approved-instrument-status-record.json")
$boundary = Read-Json (Join-Path $readiness "phase-lmax-r29-sanitized-runtime-boundary-evidence.json")
$forbidden = Read-Json (Join-Path $readiness "phase-lmax-r29-forbidden-action-validation.json")
$shutdown = Read-Json (Join-Path $readiness "phase-lmax-r29-shutdown-revert-record.json")
$nonMutation = Read-Json (Join-Path $readiness "phase-lmax-r29-post-attempt-non-mutation-validation.json")
$isolation = Read-Json (Join-Path $readiness "phase-lmax-r29-rail-isolation-validation.json")
$gate = Read-Json (Join-Path $readiness "phase-lmax-r29-decision-gate.json")
$t7Gate = Read-Json $t7GatePath
$phase7nGate = Read-Json $phase7nGatePath

Assert-Equal $r28Gate.finalDecision "LMAX_R28_FIX_PROVIDER_IMPLEMENTED_NO_EXTERNAL_ACTIVATION" "R28 decision"
...
$expectedPhrase = "I, Philippe, explicitly approve Phase LMAX-R29 for one temporary Demo read-only runtime market-data activation attempt after the FIX provider fix for GBPUSD, EURGBP, AUDUSD, and USDJPY with USDJPY caveat preserved, with no orders, no trading enablement, no scheduler, no polling, no replay, no shadow replay submit, no trading-state mutation, no production account, sanitized output only, and immediate abort authority."
Assert-Equal $approval.phase "LMAX-R29" "Approval phase"
...
Assert-True $preflight.r28DecisionConfirmed "Preflight R28"
...
foreach ($property in @("lmaxRealReadOnlySocketBoundaryProviderAvailable", "lmaxRealReadOnlyTlsBoundaryProviderAvailable", "lmaxRealReadOnlyFixFrameBoundaryProviderAvailable", "ilmaxRealReadOnlySocketBoundaryProviderHasNonTestImplementation", "ilmaxRealReadOnlyTlsBoundaryProviderHasNonTestImplementation", "ilmaxRealReadOnlyFixFrameBoundaryProviderHasNonTestImplementation", "socketProviderFixRemainsValid", "tlsProviderFixRemainsValid", "fixProviderFixResolvedR27Boundary", "lmaxExecutableReadOnlyRealDependencyProvidersAvailable", "lmaxExecutableReadOnlyRealLowLevelDependenciesAvailable", "lmaxExecutableReadOnlyLowLevelSessionStackAvailable", "lmaxExecutableReadOnlySocketSessionBoundaryAvailable", "lmaxExecutableReadOnlyFixSessionBoundaryAvailable", "lmaxExecutableReadOnlyMarketDataRequestCodecAvailable", "lmaxExecutableReadOnlyCredentialBoundaryAvailable", "publicInterfacesReadOnlyOnly", "approvedExecutableSocketBoundaryProviderAvailable", "approvedExecutableTlsBoundaryProviderAvailable", "approvedExecutableFixFrameBoundaryProviderAvailable", "testDoubleMarketDataProviderOnly", "abortBeforeExternalAction")) {
    Assert-True $stack.$property "Stack $property"
}
foreach ($property in @("orderTradingReplayMethodsAvailable", "credentialAccessSafe", "credentialValuesPrinted", "credentialValuesPersisted", "apiWorkerStartupRequired", "defaultConfigMutationRequired", "hostedServiceRequired", "backgroundWorkerRequired", "liveLauncherCreated", "approvedPathBypassed", "oldManualWrappersUsedDirectly", "approvedExecutableMarketDataFrameBoundaryProviderAvailable", "approvedExecutableCredentialConfigBoundaryProviderAvailable", "validationPassedForExecution")) {
    Assert-False $stack.$property "Stack $property"
}
Assert-Equal $stack.concreteFinalAbortCause "MarketDataProviderNotExecutable" "Concrete abort cause"
Assert-Equal $stack.validationResult "FailedMarketDataProviderNotExecutable" "Stack validation"
Assert-Equal $stack.resultClassification "LMAX_R29_FAIL_MARKETDATA_PROVIDER_NOT_EXECUTABLE" "Stack classification"
...
foreach ($instrument in $instrumentStatus.instruments) {
    ...
    Assert-Equal $instrument.status "NotAttemptedMarketDataProviderNotExecutable" "Instrument status $($instrument.symbol)"
}

Assert-True $boundary.outputSanitized "Boundary sanitized"
Assert-False $boundary.activationAttempted "Boundary activation"
Assert-Equal $boundary.finalProviderStackValidation "FailedMarketDataProviderNotExecutable" "Boundary stack validation"
Assert-Equal $boundary.concreteFinalAbortCause "MarketDataProviderNotExecutable" "Boundary abort cause"
foreach ($boundaryName in @("tcpBoundaryStatus", "tlsBoundaryStatus", "fixLogonBoundaryStatus", "marketDataRequestBoundaryStatus", "instrumentMarketDataStatus")) {
    Assert-Equal $boundary.$boundaryName "NotAttemptedMarketDataProviderNotExecutable" "Boundary $boundaryName"
}
...
Assert-True $boundary.fixProviderFixedAvailable "Boundary FIX fix"
...
foreach ($property in @("realTransportUsed", "executableSessionClientUsed", "lowLevelStackUsed", "realLowLevelDependenciesUsed", "realDependencyProvidersUsed", "socketProviderUsed", "tlsProviderUsed", "fixProviderUsed", "credentialsPrinted", "credentialsStored", "rawFixLogsStored")) {
...
foreach ($property in @("phase7ArchivesModified", "usdJpyT1T7ArtifactsModified", "gbpusdArchiveModified", "eurgbpArchiveModified", "audusdArchiveModified", "r1ToR28ArtifactsModifiedExceptReadOnlyReference", "defaultConfigChanged", "runtimeGatewayRegisteredByDefault", "persistentRuntimeEnablement")) {
...
$allowedDecisions = @(
    "LMAX_R29_PASS_TEMPORARY_READONLY_RUNTIME_ACTIVATION_SANITIZED",
    "LMAX_R29_FAIL_PREFLIGHT_ABORTED",
    "LMAX_R29_FAIL_UNSAFE_PROVIDER_STACK_REJECTED",
    "LMAX_R29_FAIL_SOCKET_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R29_FAIL_TLS_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R29_FAIL_FIX_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R29_FAIL_MARKETDATA_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R29_FAIL_CREDENTIAL_PROVIDER_NOT_APPROVED",
    "LMAX_R29_FAIL_DEMO_CREDENTIAL_CONFIG_MISSING",
    "LMAX_R29_FAIL_REQUIRES_API_WORKER_STARTUP",
    "LMAX_R29_FAIL_REQUIRES_LIVE_LAUNCHER",
    "LMAX_R29_FAIL_RUNTIME_ACTIVATION_BOUNDARY",
    "LMAX_R29_FAIL_SESSION_OR_MARKETDATA_BOUNDARY",
    "LMAX_R29_FAIL_SAFETY_CONSTRAINT",
    "LMAX_R29_INCONCLUSIVE_SANITIZED_EVIDENCE"
)
Assert-In $gate.finalDecision $allowedDecisions "R29 decision classification"
Assert-Equal $gate.finalDecision "LMAX_R29_FAIL_MARKETDATA_PROVIDER_NOT_EXECUTABLE" "R29 final decision"
Assert-Equal $gate.concreteFinalAbortCause "MarketDataProviderNotExecutable" "Gate abort cause"
...
Assert-False $gate.r30Authorized "R30 authorization"
...
$fixProviderPath = Join-Path $RepoRoot "src\QQ.Production.Intraday.Infrastructure.Lmax\LmaxRealReadOnlyFixFrameBoundaryProvider.cs"
foreach ($path in @($providerPath, $socketProviderPath, $tlsProviderPath, $fixProviderPath)) {
...
$fixProviderCode = Get-Content -LiteralPath $fixProviderPath -Raw
foreach ($needle in @("ILmaxRealReadOnlyMarketDataFrameBoundaryProvider", "LmaxRealMarketDataFrameProvider", "LmaxRealReadOnlyDependencyProviderFactory")) {
...
Assert-TextContains $fixProviderCode "class LmaxRealReadOnlyFixFrameBoundaryProvider : ILmaxRealReadOnlyFixFrameBoundaryProvider" "R28 FIX implementation"
...
$fixImplementations = Select-String -Path (Join-Path $RepoRoot "src\QQ.Production.Intraday.Infrastructure.Lmax\*.cs") -Pattern "class\s+LmaxRealReadOnlyFixFrameBoundaryProvider\s*:\s*ILmaxRealReadOnlyFixFrameBoundaryProvider"
if (@($fixImplementations).Count -ne 1) { throw "Expected exactly one non-test FIX provider implementation." }
$marketDataImplementations = Select-String -Path (Join-Path $RepoRoot "src\QQ.Production.Intraday.Infrastructure.Lmax\*.cs") -Pattern "class\s+\w+\s*:\s*ILmaxRealReadOnlyMarketDataFrameBoundaryProvider"
if (@($marketDataImplementations).Count -ne 0) { throw "R29 expected no approved production ILmaxRealReadOnlyMarketDataFrameBoundaryProvider implementation, but found one." }
...
foreach ($pattern in @("LmaxRealReadOnlyDependencyProviderFactory", "LmaxRealReadOnlySocketBoundaryProvider", "LmaxRealReadOnlyTlsBoundaryProvider", "LmaxRealReadOnlyFixFrameBoundaryProvider", "LmaxRealMarketDataFrameProvider", "phase-lmax-r29")) {
    Assert-TextNotContainsPattern $program $pattern "API wiring must not include R29 path"
    Assert-TextNotContainsPattern $worker $pattern "Worker wiring must not include R29 path"
    Assert-TextNotContainsPattern $settings $pattern "Default config must not include R29 path"
}

$validation = [ordered]@{
    phase = "LMAX-R29"
    validator = "scripts/check-lmax-r29-temporary-readonly-runtime-gate.ps1"
    validationPassed = $true
    finalDecision = "LMAX_R29_FAIL_MARKETDATA_PROVIDER_NOT_EXECUTABLE"
    r28DecisionConfirmed = $true
    operatorApprovalExact = $true
    preflightPassed = $true
    preflightAborted = $false
    finalProviderStackValidation = "FailedMarketDataProviderNotExecutable"
    concreteFinalAbortCause = "MarketDataProviderNotExecutable"
    socketProviderFixRemainsValid = $true
    tlsProviderFixRemainsValid = $true
    fixProviderFixResolvedR27Boundary = $true
    ...
    recommendedNextPhase = "Phase LMAX-R30 - Concrete Final Execution Boundary Fix"
}

$validationPath = Join-Path $readiness "phase-lmax-r29-gate-validation.json"
...
Write-Host "LMAX-R29 gate validation passed."
