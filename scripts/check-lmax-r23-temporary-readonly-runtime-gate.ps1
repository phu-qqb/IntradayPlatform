...
$requiredPrior = @(
    ...
    "phase-lmax-r22-gate-validation.json"
)

$requiredR23 = @(
    "phase-lmax-r23-operator-approval-record.json",
    "phase-lmax-r23-preflight-gate.json",
    "phase-lmax-r23-final-provider-stack-validation.json",
    "phase-lmax-r23-temporary-runtime-activation-record.json",
    "phase-lmax-r23-approved-instrument-status-record.json",
    "phase-lmax-r23-sanitized-runtime-boundary-evidence.json",
    "phase-lmax-r23-forbidden-action-validation.json",
    "phase-lmax-r23-shutdown-revert-record.json",
    "phase-lmax-r23-post-attempt-non-mutation-validation.json",
    "phase-lmax-r23-rail-isolation-validation.json",
    "phase-lmax-r23-decision-gate.json",
    "phase-lmax-r23-temporary-readonly-runtime-report.md",
    "phase-lmax-r23-operator-note.md"
)

Write-Host "LMAX-R23 Temporary Read-Only Runtime Gate Validator"
Write-Host "This validator verifies R23 artifacts and confirms no forbidden action occurred."

Assert-RequiredArtifacts $readiness $requiredPrior
Assert-RequiredArtifacts $readiness $requiredR23
...
$r22Gate = Read-Json (Join-Path $readiness "phase-lmax-r22-decision-gate.json")
$approval = Read-Json (Join-Path $readiness "phase-lmax-r23-operator-approval-record.json")
$preflight = Read-Json (Join-Path $readiness "phase-lmax-r23-preflight-gate.json")
$stack = Read-Json (Join-Path $readiness "phase-lmax-r23-final-provider-stack-validation.json")
$activation = Read-Json (Join-Path $readiness "phase-lmax-r23-temporary-runtime-activation-record.json")
$instrumentStatus = Read-Json (Join-Path $readiness "phase-lmax-r23-approved-instrument-status-record.json")
$boundary = Read-Json (Join-Path $readiness "phase-lmax-r23-sanitized-runtime-boundary-evidence.json")
$forbidden = Read-Json (Join-Path $readiness "phase-lmax-r23-forbidden-action-validation.json")
$shutdown = Read-Json (Join-Path $readiness "phase-lmax-r23-shutdown-revert-record.json")
$nonMutation = Read-Json (Join-Path $readiness "phase-lmax-r23-post-attempt-non-mutation-validation.json")
$isolation = Read-Json (Join-Path $readiness "phase-lmax-r23-rail-isolation-validation.json")
$gate = Read-Json (Join-Path $readiness "phase-lmax-r23-decision-gate.json")
$t7Gate = Read-Json $t7GatePath
$phase7nGate = Read-Json $phase7nGatePath

Assert-Equal $r22Gate.finalDecision "LMAX_R22_REAL_DEPENDENCY_PROVIDERS_IMPLEMENTED_WITH_TEST_DOUBLES_NO_EXTERNAL_ACTIVATION" "R22 decision"
...
$expectedPhrase = "I, Philippe, explicitly approve Phase LMAX-R23 for one temporary Demo read-only runtime market-data activation attempt using the real dependency providers for GBPUSD, EURGBP, AUDUSD, and USDJPY with USDJPY caveat preserved, with no orders, no trading enablement, no scheduler, no polling, no replay, no shadow replay submit, no trading-state mutation, no production account, sanitized output only, and immediate abort authority."
Assert-Equal $approval.phase "LMAX-R23" "Approval phase"
...
Assert-True $preflight.r22DecisionConfirmed "Preflight R22"
...
foreach ($property in @("lmaxExecutableReadOnlyRealDependencyProvidersAvailable", "lmaxExecutableReadOnlyRealLowLevelDependenciesAvailable", "lmaxExecutableReadOnlyLowLevelSessionStackAvailable", "lmaxExecutableReadOnlySocketSessionBoundaryAvailable", "lmaxExecutableReadOnlyFixSessionBoundaryAvailable", "lmaxExecutableReadOnlyMarketDataRequestCodecAvailable", "lmaxExecutableReadOnlyCredentialBoundaryAvailable", "publicInterfacesReadOnlyOnly", "testDoubleProvidersOnly", "abortBeforeExternalAction")) {
    Assert-True $stack.$property "Stack $property"
}
foreach ($property in @("orderTradingReplayMethodsAvailable", "credentialAccessSafe", "credentialValuesPrinted", "credentialValuesPersisted", "apiWorkerStartupRequired", "defaultConfigMutationRequired", "hostedServiceRequired", "backgroundWorkerRequired", "liveLauncherCreated", "approvedPathBypassed", "oldManualWrappersUsedDirectly", "approvedExecutableSocketBoundaryProviderAvailable", "approvedExecutableTlsBoundaryProviderAvailable", "approvedExecutableFixFrameBoundaryProviderAvailable", "approvedExecutableMarketDataFrameBoundaryProviderAvailable", "approvedExecutableCredentialConfigBoundaryProviderAvailable", "validationPassedForExecution")) {
    Assert-False $stack.$property "Stack $property"
}
Assert-Equal $stack.concreteFinalAbortCause "SocketProviderNotExecutable" "Concrete abort cause"
Assert-Equal $stack.validationResult "FailedSocketProviderNotExecutable" "Stack validation"
Assert-Equal $stack.resultClassification "LMAX_R23_FAIL_SOCKET_PROVIDER_NOT_EXECUTABLE" "Stack classification"
...
Assert-Equal $boundary.finalProviderStackValidation "FailedSocketProviderNotExecutable" "Boundary stack validation"
Assert-Equal $boundary.concreteFinalAbortCause "SocketProviderNotExecutable" "Boundary abort cause"
foreach ($boundaryName in @("tcpBoundaryStatus", "tlsBoundaryStatus", "fixLogonBoundaryStatus", "marketDataRequestBoundaryStatus", "instrumentMarketDataStatus")) {
    Assert-Equal $boundary.$boundaryName "NotAttemptedSocketProviderNotExecutable" "Boundary $boundaryName"
}
...
foreach ($property in @("realTransportUsed", "executableSessionClientUsed", "lowLevelStackUsed", "realLowLevelDependenciesUsed", "realDependencyProvidersUsed", "credentialsPrinted", "credentialsStored", "rawFixLogsStored")) {
...
foreach ($property in @("phase7ArchivesModified", "usdJpyT1T7ArtifactsModified", "gbpusdArchiveModified", "eurgbpArchiveModified", "audusdArchiveModified", "r1ToR22ArtifactsModifiedExceptReadOnlyReference", "defaultConfigChanged", "runtimeGatewayRegisteredByDefault", "persistentRuntimeEnablement")) {
...
$allowedDecisions = @(
    "LMAX_R23_PASS_TEMPORARY_READONLY_RUNTIME_ACTIVATION_SANITIZED",
    "LMAX_R23_FAIL_PREFLIGHT_ABORTED",
    "LMAX_R23_FAIL_UNSAFE_PROVIDER_STACK_REJECTED",
    "LMAX_R23_FAIL_CREDENTIAL_PROVIDER_NOT_APPROVED",
    "LMAX_R23_FAIL_DEMO_CREDENTIAL_CONFIG_MISSING",
    "LMAX_R23_FAIL_SOCKET_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R23_FAIL_TLS_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R23_FAIL_FIX_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R23_FAIL_MARKETDATA_PROVIDER_NOT_EXECUTABLE",
    "LMAX_R23_FAIL_REQUIRES_API_WORKER_STARTUP",
    "LMAX_R23_FAIL_REQUIRES_LIVE_LAUNCHER",
    "LMAX_R23_FAIL_RUNTIME_ACTIVATION_BOUNDARY",
    "LMAX_R23_FAIL_SESSION_OR_MARKETDATA_BOUNDARY",
    "LMAX_R23_FAIL_SAFETY_CONSTRAINT",
    "LMAX_R23_INCONCLUSIVE_SANITIZED_EVIDENCE"
)
Assert-In $gate.finalDecision $allowedDecisions "R23 decision classification"
Assert-Equal $gate.finalDecision "LMAX_R23_FAIL_SOCKET_PROVIDER_NOT_EXECUTABLE" "R23 final decision"
Assert-Equal $gate.concreteFinalAbortCause "SocketProviderNotExecutable" "Gate abort cause"
...
foreach ($needle in @("ILmaxRealReadOnlySocketBoundaryProvider", "LmaxRealSocketProvider", "LmaxRealReadOnlyDependencyProviderFactory")) {
    Assert-TextContains $providerCode $needle "R22 provider code token"
}

$productionImplementations = Select-String -Path (Join-Path $RepoRoot "src\QQ.Production.Intraday.Infrastructure.Lmax\*.cs") -Pattern ":\s*ILmaxRealReadOnlySocketBoundaryProvider" | Where-Object { $_.Path -notlike "*LmaxExecutableReadOnlyRealDependencyProviders.cs" }
if (@($productionImplementations).Count -ne 0) {
    throw "R23 expected no approved production ILmaxRealReadOnlySocketBoundaryProvider implementation, but found one."
}
...
foreach ($pattern in @("LmaxRealReadOnlyDependencyProviderFactory", "LmaxRealSocketProvider", "phase-lmax-r23")) {
    Assert-TextNotContainsPattern $program $pattern "API wiring must not include R23 path"
    Assert-TextNotContainsPattern $worker $pattern "Worker wiring must not include R23 path"
    Assert-TextNotContainsPattern $settings $pattern "Default config must not include R23 path"
}

$validation = [ordered]@{
    phase = "LMAX-R23"
    validator = "scripts/check-lmax-r23-temporary-readonly-runtime-gate.ps1"
    validationPassed = $true
    finalDecision = "LMAX_R23_FAIL_SOCKET_PROVIDER_NOT_EXECUTABLE"
    r22DecisionConfirmed = $true
    operatorApprovalExact = $true
    preflightPassed = $true
    preflightAborted = $false
    finalProviderStackValidation = "FailedSocketProviderNotExecutable"
    concreteFinalAbortCause = "SocketProviderNotExecutable"
    ...
    recommendedNextPhase = "Phase LMAX-R24 - Concrete Final Execution Boundary Fix"
}

$validationPath = Join-Path $readiness "phase-lmax-r23-gate-validation.json"
...
Write-Host "LMAX-R23 gate validation passed."
