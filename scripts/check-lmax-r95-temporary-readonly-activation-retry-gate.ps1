...
$summary = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-temporary-readonly-activation-retry-summary.json')
$preflight = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-preflight-result.json')
$endpoint = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-demo-endpoint-binding-evidence.json')
$socket = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-socket-connector-evidence.json')
$tls = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-tls-boundary-evidence.json')
$tlsClassification = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-tls-classification-evidence.json')
$credential = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-fix-credential-material-evidence.json')
$fixWrite = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-fix-logon-frame-write-evidence.json')
$fix = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-fix-session-boundary-evidence.json')
$market = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-marketdata-request-evidence.json')
$trace = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-operational-invocation-trace.json')
$boundary = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-boundary-evidence.json')
$marketResult = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-marketdata-sanitized-result.json')
$forbidden = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-forbidden-actions-audit.json')
$apiWorker = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-api-worker-fake-gateway-audit.json')
$usdJpy = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-usdjpy-caveat-preservation.json')
$shutdown = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-shutdown-revert-evidence.json')
$next = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-next-phase-recommendation.json')
$gate = Read-Json (Join-Path $artifactRoot 'phase-lmax-r95-gate-validation.json')
$r94 = Read-Json (Join-Path $artifactRoot 'phase-lmax-r94-tls-runtime-boundary-root-cause-summary.json')

$allowedClassifications = @(
    'LMAX_R95_PASS_TEMPORARY_READONLY_RUNTIME_ACTIVATION_SANITIZED',
    'LMAX_R95_FAIL_OPERATOR_APPROVAL_MISSING_OR_MISMATCHED',
    'LMAX_R95_FAIL_R94_SUCCESS_EVIDENCE_MISSING',
    'LMAX_R95_FAIL_DEMO_ENDPOINT_BINDING_REGRESSION',
    'LMAX_R95_FAIL_PLACEHOLDER_HOST_USED',
    'LMAX_R95_FAIL_SOCKET_CONNECTOR_BINDING_REGRESSION',
    'LMAX_R95_FAIL_TLS_PROGRESSION_BINDING_REGRESSION',
    'LMAX_R95_FAIL_TLS_CLASSIFICATION_INSTRUMENTATION_REGRESSION',
    'LMAX_R95_FAIL_FIX_PROGRESSION_BINDING_REGRESSION',
    'LMAX_R95_FAIL_FIX_CREDENTIAL_MATERIAL_BINDING_REGRESSION',
    'LMAX_R95_FAIL_FIX_LOGON_FRAME_WRITE_BINDING_REGRESSION',
    'LMAX_R95_FAIL_FIX_WRITER_SUPPORTS_ORDER_FRAMES',
    'LMAX_R95_FAIL_REAL_SECRET_MATERIAL_NOT_ALLOWED_FOR_APPROVED_RETRY',
    'LMAX_R95_FAIL_REAL_SECRET_MATERIAL_NOT_LOADED_FOR_APPROVED_RETRY',
    'LMAX_R95_FAIL_FIX_ATTEMPT_ALLOWED_WITHOUT_TLS_SUCCESS',
    'LMAX_R95_FAIL_MARKETDATA_ATTEMPT_ALLOWED_WITHOUT_FIX_SUCCESS',
    'LMAX_R95_FAIL_MANUAL_EXECUTION_SURFACE_REGRESSION',
    'LMAX_R95_FAIL_REAL_BOUNDED_ADAPTER_BINDING_REGRESSION',
    'LMAX_R95_FAIL_NO_EXTERNAL_ADAPTER_USED_INSTEAD_OF_REAL_BOUNDARY_ADAPTER',
    'LMAX_R95_FAIL_EXECUTE_ONCE_NOT_INVOKED',
    'LMAX_R95_FAIL_PRE_EXTERNAL_APPROVAL_OR_COMPOSITION_REGRESSION',
    'LMAX_R95_FAIL_CREDENTIAL_CONFIG_MISSING',
    'LMAX_R95_FAIL_CREDENTIAL_CONFIG_NOT_APPROVED',
    'LMAX_R95_FAIL_CREDENTIAL_CONFIG_BINDING_REGRESSION',
    'LMAX_R95_FAIL_PRODUCTION_ACCOUNT_RISK',
    'LMAX_R95_FAIL_API_WORKER_GATEWAY_REGRESSION',
    'LMAX_R95_FAIL_LIVE_LAUNCHER_OR_BACKGROUND_SERVICE_RISK',
    'LMAX_R95_FAIL_FORBIDDEN_ACTION_RISK',
    'LMAX_R95_FAIL_USDJPY_CAVEAT_WEAKENED',
    'LMAX_R95_FAIL_PREFLIGHT_ABORTED',
    'LMAX_R95_FAIL_TCP_SOCKET_BOUNDARY',
    'LMAX_R95_FAIL_TLS_BOUNDARY',
    'LMAX_R95_FAIL_FIX_LOGON_OR_SESSION_BOUNDARY',
    'LMAX_R95_FAIL_MARKETDATA_REQUEST_BOUNDARY',
    'LMAX_R95_FAIL_MARKETDATA_RESPONSE_BOUNDARY',
    'LMAX_R95_FAIL_RUNTIME_BOUNDARY_INCONCLUSIVE',
    'LMAX_R95_FAIL_SHUTDOWN_REVERT_INCOMPLETE',
    'LMAX_R95_FAIL_SANITIZATION_OR_SECRET_LEAK_RISK',
    'LMAX_R95_FAIL_BUILD_OR_TESTS'
)

if ($allowedClassifications -notcontains $summary.classification) {
    throw "Unexpected R95 classification: $($summary.classification)"
}

Require-Equal $summary.classification 'LMAX_R95_FAIL_TLS_BOUNDARY' 'R95 classification mismatch.'
Require-Equal $r94.classification 'LMAX_R94_PASS_TLS_HANDSHAKE_EXCEPTION_SANITIZED_CAUSE_INCONCLUSIVE_NO_EXTERNAL_ACTIVATION' 'R94 success evidence missing or mismatched.'
...
Require-True $summary.retryPhaseReservationPassed 'LMAX-R95 retry phase reservation did not pass.'
...
Require-False $summary.tlsSucceeded 'R95 should not claim TLS success for this evidence pack.'
...
Require-False $summary.tlsTimedOut 'R95 TLS evidence should not be classified as timeout.'
...
Require-True $preflight.r94SuccessEvidencePresent 'R94 evidence missing.'
...
Require-Equal $next.nextRecommendedPhase 'LMAX-R96' 'Next phase recommendation missing or incorrect.'
Require-Equal $next.nextRecommendedTitle 'TLS Runtime Boundary Root-Cause Pack' 'Next phase title mismatch.'
Require-True $next.r96MustNotPerformActivation 'R96 no-activation constraint missing.'
...
$artifactText = Get-ChildItem -LiteralPath $artifactRoot -Filter 'phase-lmax-r95-*' -File |
    Where-Object { $_.Name -ne 'phase-lmax-r95-operator-approval-note.md' } |
...
foreach ($pattern in $forbiddenPatterns) {
    if ($artifactText -match $pattern) {
        throw "Forbidden sensitive or trading pattern found in R95 artifacts: $pattern"
    }
}

Write-Output 'LMAX_R95_VALIDATION_PASS'
