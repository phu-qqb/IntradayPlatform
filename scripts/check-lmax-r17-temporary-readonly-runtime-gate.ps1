...
$requiredPrior = @(
    ...
    "phase-lmax-r16-gate-validation.json"
)

$requiredR17 = @(
    "phase-lmax-r17-operator-approval-record.json",
    "phase-lmax-r17-preflight-gate.json",
    "phase-lmax-r17-executable-session-client-validation.json",
    "phase-lmax-r17-temporary-runtime-activation-record.json",
    "phase-lmax-r17-approved-instrument-status-record.json",
    "phase-lmax-r17-sanitized-runtime-boundary-evidence.json",
    "phase-lmax-r17-forbidden-action-validation.json",
    "phase-lmax-r17-shutdown-revert-record.json",
    "phase-lmax-r17-post-attempt-non-mutation-validation.json",
    "phase-lmax-r17-rail-isolation-validation.json",
    "phase-lmax-r17-decision-gate.json",
    "phase-lmax-r17-temporary-readonly-runtime-report.md",
    "phase-lmax-r17-operator-note.md"
)

Write-Host "LMAX-R17 Temporary Read-Only Runtime Gate Validator"
...
Assert-RequiredArtifacts $readiness $requiredR17
...
$r16Gate = Read-Json (Join-Path $readiness "phase-lmax-r16-decision-gate.json")
$approval = Read-Json (Join-Path $readiness "phase-lmax-r17-operator-approval-record.json")
$preflight = Read-Json (Join-Path $readiness "phase-lmax-r17-preflight-gate.json")
$sessionClient = Read-Json (Join-Path $readiness "phase-lmax-r17-executable-session-client-validation.json")
$activation = Read-Json (Join-Path $readiness "phase-lmax-r17-temporary-runtime-activation-record.json")
$instrumentStatus = Read-Json (Join-Path $readiness "phase-lmax-r17-approved-instrument-status-record.json")
$boundary = Read-Json (Join-Path $readiness "phase-lmax-r17-sanitized-runtime-boundary-evidence.json")
$forbidden = Read-Json (Join-Path $readiness "phase-lmax-r17-forbidden-action-validation.json")
$shutdown = Read-Json (Join-Path $readiness "phase-lmax-r17-shutdown-revert-record.json")
$nonMutation = Read-Json (Join-Path $readiness "phase-lmax-r17-post-attempt-non-mutation-validation.json")
$isolation = Read-Json (Join-Path $readiness "phase-lmax-r17-rail-isolation-validation.json")
$gate = Read-Json (Join-Path $readiness "phase-lmax-r17-decision-gate.json")
$t7Gate = Read-Json $t7GatePath
$phase7nGate = Read-Json $phase7nGatePath

Assert-Equal $r16Gate.finalDecision "LMAX_R16_EXECUTABLE_READONLY_SESSION_CLIENT_IMPLEMENTED_WITH_TEST_DOUBLES_NO_EXTERNAL_ACTIVATION" "R16 decision"
...
$expectedPhrase = "I, Philippe, explicitly approve Phase LMAX-R17 for one temporary Demo read-only runtime market-data activation attempt using the executable session client for GBPUSD, EURGBP, AUDUSD, and USDJPY with USDJPY caveat preserved, with no orders, no trading enablement, no scheduler, no polling, no replay, no shadow replay submit, no trading-state mutation, no production account, sanitized output only, and immediate abort authority."
Assert-Equal $approval.phase "LMAX-R17" "Approval phase"
...
Assert-True $preflight.r16DecisionConfirmed "Preflight R16"
...
Assert-True $sessionClient.sessionClientClassPresent "Session client class present"
Assert-Equal $sessionClient.sessionClientClass "LmaxExecutableReadOnlyMarketDataSessionClient" "Session client class"
Assert-Equal $sessionClient.sessionClientInterface "ILmaxReadOnlyMarketDataSessionClient" "Session client interface"
Assert-True $sessionClient.sessionClientReadOnlyInterfaceConfirmed "Session client read-only interface"
Assert-False $sessionClient.orderTradingReplayMethodsAvailable "Session client forbidden methods"
Assert-True $sessionClient.lowLevelSocketBoundaryAbstractionPresent "Session client socket abstraction"
Assert-True $sessionClient.lowLevelFixBoundaryAbstractionPresent "Session client FIX abstraction"
Assert-True $sessionClient.lowLevelMarketDataCodecAbstractionPresent "Session client market-data abstraction"
Assert-True $sessionClient.r16ExercisedWithTestDoublesOnly "Session client test doubles"
Assert-False $sessionClient.approvedExecutableLowLevelSessionStackAvailable "Session executable low-level stack"
Assert-False $sessionClient.demoReadOnlyExecutableLowLevelSessionAvailable "Session demo low-level stack"
Assert-False $sessionClient.safeCredentialAccessMechanismExecutableForR17 "Session credential execution"
Assert-False $sessionClient.credentialValuesPrinted "Session printed"
Assert-False $sessionClient.credentialValuesPersisted "Session persisted"
Assert-False $sessionClient.apiWorkerStartupRequired "Session API/Worker"
Assert-False $sessionClient.defaultConfigMutationRequired "Session config mutation"
Assert-False $sessionClient.hostedServiceRequired "Session hosted"
Assert-False $sessionClient.backgroundWorkerRequired "Session background"
Assert-False $sessionClient.liveLauncherCreated "Session launcher"
Assert-False $sessionClient.oldManualWrappersUsedDirectly "Session wrappers"
Assert-False $sessionClient.approvedPathBypassed "Session bypass"
Assert-Equal $sessionClient.validationResult "FailedNoApprovedExecutableLowLevelSessionStack" "Session validation"
Assert-False $sessionClient.validationPassedForExecution "Session execution validation"
Assert-True $sessionClient.abortBeforeExternalAction "Session abort"
Assert-Equal $sessionClient.resultClassification "LMAX_R17_FAIL_NO_EXECUTABLE_SESSION_CLIENT_AVAILABLE" "Session classification"
...
Assert-True $activation.executableSessionClientValidationCompleted "Activation session validation"
Assert-False $activation.executableSessionClientValidationPassedForExecution "Activation session passed"
...
Assert-Equal $boundary.executableSessionClientValidation "FailedNoApprovedExecutableLowLevelSessionStack" "Boundary session validation"
...
foreach ($property in @("evidenceArchivesModified", "validatedRailsModified", "phase7ArchiveModified", "usdJpyT1T7ArtifactsModified", "r1ArtifactsModified", "r2ArtifactsModified", "r3ArtifactsModified", "r4ArtifactsModified", "r5ArtifactsModified", "r6ArtifactsModified", "r7ArtifactsModified", "r8ArtifactsModified", "r9ArtifactsModified", "r10ArtifactsModified", "r11ArtifactsModified", "r12ArtifactsModified", "r13ArtifactsModified", "r14ArtifactsModified", "r15ArtifactsModified", "r16ArtifactsModified", "gbpusdArchiveModified", "eurgbpArchiveModified", "audusdArchiveModified", "nonApprovedInstrumentTouched")) {
    Assert-False $isolation.$property "Isolation $property"
}

$allowedDecisions = @(
    "LMAX_R17_PASS_TEMPORARY_READONLY_RUNTIME_ACTIVATION_SANITIZED",
    "LMAX_R17_FAIL_PREFLIGHT_ABORTED",
    "LMAX_R17_FAIL_NO_EXECUTABLE_SESSION_CLIENT_AVAILABLE",
    "LMAX_R17_FAIL_UNSAFE_SESSION_CLIENT_REJECTED",
    "LMAX_R17_FAIL_RUNTIME_ACTIVATION_BOUNDARY",
    "LMAX_R17_FAIL_SESSION_OR_MARKETDATA_BOUNDARY",
    "LMAX_R17_FAIL_SAFETY_CONSTRAINT",
    "LMAX_R17_INCONCLUSIVE_SANITIZED_EVIDENCE"
)
Assert-In $gate.finalDecision $allowedDecisions "R17 final decision"
Assert-Equal $gate.finalDecision "LMAX_R17_FAIL_NO_EXECUTABLE_SESSION_CLIENT_AVAILABLE" "R17 expected decision"
Assert-Equal $gate.resultClassification "LMAX_R17_FAIL_NO_EXECUTABLE_SESSION_CLIENT_AVAILABLE" "R17 classification"
...
Assert-True $gate.executableSessionClientValidationCompleted "Gate session validation"
Assert-False $gate.executableSessionClientValidationPassedForExecution "Gate session execution"
Assert-Equal $gate.executableSessionClientValidation "FailedNoApprovedExecutableLowLevelSessionStack" "Gate session validation value"
...
if (-not (Test-Path -LiteralPath $clientPath)) {
    throw "Missing R16 executable session client: $clientPath"
}
...
foreach ($requiredToken in @("LmaxExecutableReadOnlyMarketDataSessionClient", "ILmaxReadOnlySocketSessionBoundary", "ILmaxReadOnlyFixSessionBoundary", "ILmaxReadOnlyMarketDataRequestCodec", "ILmaxReadOnlyMarketDataSessionClient")) {
    if ($clientSource -notmatch [regex]::Escape($requiredToken)) {
        throw "Executable session client source missing token: $requiredToken"
    }
}
foreach ($forbiddenToken in @("TcpClient", "System.Net.Sockets", "new Socket", "SslStream", "NetworkStream", "NewOrderSingle", "OrderCancelRequest", "OrderStatusRequest", "TradeCaptureReportRequest", "SubmitOrder")) {
    if ($clientSource -match [regex]::Escape($forbiddenToken)) {
        throw "Executable session client source references forbidden token: $forbiddenToken"
    }
}
...
if ($programText -match 'LmaxExecutableReadOnlyMarketDataSessionClient|ILmaxReadOnlySocketSessionBoundary|ILmaxReadOnlyFixSessionBoundary|ILmaxReadOnlyMarketDataRequestCodec') {
    throw "API Program.cs contains R17/R16 session client wiring."
}
if ($workerText -match 'LmaxExecutableReadOnlyMarketDataSessionClient|ILmaxReadOnlySocketSessionBoundary|ILmaxReadOnlyFixSessionBoundary|ILmaxReadOnlyMarketDataRequestCodec') {
    throw "Worker Program.cs contains R17/R16 session client wiring."
}
...
$reportPath = Join-Path $readiness "phase-lmax-r17-temporary-readonly-runtime-report.md"
$reportText = Get-Content -LiteralPath $reportPath -Raw
foreach ($heading in @(
    "Executive summary",
    "Operator approval",
    "Scope and constraints",
    "R16 executable session client basis",
    "Preflight result",
    "Executable session client validation",
    ...
    if ($reportText -notmatch [regex]::Escape($heading)) {
        throw "R17 report missing required section: $heading"
    }
}
...
$validation = [ordered]@{
    ...
    validator = "scripts/check-lmax-r17-temporary-readonly-runtime-gate.ps1"
    requiredArtifactsPresent = $true
    priorArtifactsPresent = $true
    r16DecisionConfirmed = $true
    operatorApprovalExact = $true
    preflightPassed = $true
    preflightAborted = $false
    executableSessionClientValidation = "FailedNoApprovedExecutableLowLevelSessionStack"
    executableSessionClientValidationPassedForExecution = $false
...
$validationPath = Join-Path $readiness "phase-lmax-r17-gate-validation.json"
