param(
    ...
    [string]$Phase6ZaFinalReadinessFile = "artifacts/lmax-readonly-runtime-securityid-planning/final-readiness/lmax-readonly-additional-instrument-final-readiness-USDJPY-20260509-175849.json"
)
...
function Read-JsonForGate([string]$PathValue, [string]$Label) {
    ...
    if ($raw -match $script:sensitivePattern) { Add-Result $Label "No sensitive content" "FAIL" "Credential-shaped or raw FIX content found." } else { Add-Result $Label "No sensitive content" "PASS" "No credential-shaped or raw FIX content." }
...
Write-Host "LMAX Read-Only Runtime Phase 7H2 Additional-Instrument Final Pre-Run Gate"
...
foreach ($required in @(
    @{ name = "Phase 7H2 generic final pre-run model"; path = "src/QQ.Production.Intraday.Infrastructure.Lmax/LmaxReadOnlyAdditionalInstrumentFinalPreRunGate.cs" },
    @{ name = "Phase 7H2 builder"; path = "scripts/new-lmax-readonly-additional-instrument-final-pre-run-gate.ps1" },
    @{ name = "Phase 7H2 gate script"; path = "scripts/check-lmax-readonly-runtime-phase7h2-additional-instrument-final-prerun-gate.ps1" },
    @{ name = "Phase 7H2 tests"; path = "tests/QQ.Production.Intraday.Tests.Unit/LmaxReadOnlyAdditionalInstrumentFinalPreRunGateTests.cs" },
    @{ name = "Phase 7H wrapper"; path = "scripts/run-lmax-readonly-runtime-demo-additional-instrument-snapshot-once.ps1" }
)) {
...
if (-not [string]::IsNullOrWhiteSpace($Phase6ZaFinalReadinessFile)) {
    $old = Read-JsonForGate $Phase6ZaFinalReadinessFile "Phase6ZAReadiness"
    if ($null -ne $old) {
        if ([string]$old.readinessDecision -eq "PASS" -and -not ($old.PSObject.Properties.Name -contains "oneInstrumentAtATime")) {
            Add-Result "Phase6ZAReadiness" "Remains rejected by Phase 7H wrapper contract" "PASS" "Generic final-readiness has no oneInstrumentAtATime field and is not a Phase 7H final pre-run gate."
        } else {
            Add-Result "Phase6ZAReadiness" "Remains rejected by Phase 7H wrapper contract" "FAIL" "Old readiness unexpectedly has Phase 7H wrapper fields."
        }
    }
}

if ([string]::IsNullOrWhiteSpace($FinalPreRunGateFile)) {
    Add-Result "FinalPreRunGate" "Final pre-run artifact supplied" "WARN" "No final pre-run gate supplied; model/script/source gate mode."
} else {
    ...
    if ($null -ne $gate) {
        ...
        if ($expected.ContainsKey($symbol) -and [string]$gate.slashSymbol -eq $expected[$symbol].slashSymbol -and [string]$gate.planningSecurityId -eq $expected[$symbol].securityId -and [string]$gate.securityIdSource -eq "8") {
            Add-Result "FinalPreRunGate" "Supported instrument identity" "PASS" "$symbol / $($gate.planningSecurityId) / source 8."
        } else {
            Add-Result "FinalPreRunGate" "Supported instrument identity" "FAIL" "Unexpected instrument/security identity."
        }
        ...
        if ([string]$gate.finalDecision -eq "PASS") { Add-Result "FinalPreRunGate" "Decision PASS" "PASS" "Final pre-run decision PASS." } else { Add-Result "FinalPreRunGate" "Decision PASS" "FAIL" "Final pre-run decision $($gate.finalDecision)." }
    }
}
...
$outPath = Join-Path $outDir "phase7h2-additional-instrument-final-prerun-gate.json"
[ordered]@{
