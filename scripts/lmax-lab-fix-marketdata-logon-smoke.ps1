...
dotnet run --project $project --no-build --no-restore -- fix-marketdata-logon-smoke --environment=$EnvironmentName --allow-external-connections=$($AllowExternalConnections.IsPresent)
