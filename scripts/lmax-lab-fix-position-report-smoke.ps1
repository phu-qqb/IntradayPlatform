...
dotnet run --project $project --no-build --no-restore -- fix-position-report-smoke --environment=$EnvironmentName --allow-external-connections=$($AllowExternalConnections.IsPresent) --show-fix-messages=$($ShowFixMessages.IsPresent)
