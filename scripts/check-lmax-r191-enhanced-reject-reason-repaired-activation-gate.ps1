...
$activation = Read-Json 'phase-lmax-r191-enhanced-reject-reason-repaired-activation.json'
$preflight = Read-Json 'phase-lmax-r191-preflight-result.json'
$selected = Read-Json 'phase-lmax-r191-selected-profile-evidence.json'
$state = Read-Json 'phase-lmax-r191-state-evidence.json'
$extraction = Read-Json 'phase-lmax-r191-enhanced-reject-extraction-evidence.json'
$boundary = Read-Json 'phase-lmax-r191-boundary-evidence.json'
$request = Read-Json 'phase-lmax-r191-marketdata-request-evidence.json'
$response = Read-Json 'phase-lmax-r191-marketdata-response-evidence.json'
$sanitized = Read-Json 'phase-lmax-r191-sanitized-result.json'
$shutdown = Read-Json 'phase-lmax-r191-shutdown-revert-evidence.json'
$sanitization = Read-Json 'phase-lmax-r191-sanitization-audit.json'
$forbidden = Read-Json 'phase-lmax-r191-forbidden-actions-audit.json'
$apiWorker = Read-Json 'phase-lmax-r191-api-worker-fake-gateway-audit.json'
$approvedUniverse = Read-Json 'phase-lmax-r191-approved-universe-preservation.json'
$usdJpy = Read-Json 'phase-lmax-r191-usdjpy-caveat-preservation.json'
$next = Read-Json 'phase-lmax-r191-next-phase-recommendation.json'
$gate = Read-Json 'phase-lmax-r191-gate-validation.json'
$r188 = Read-Json 'phase-lmax-r188-enhanced-reject-extraction-propagation-review.json'
$r189 = Read-Json 'phase-lmax-r189-enhanced-reject-extraction-repaired-retry-readiness.json'

Assert-True ($activation.classification -in @(
    'LMAX_R191_PASS_ENHANCED_REJECT_REASON_RUNTIME_ACTIVATION_SANITIZED',
    'LMAX_R191_FAIL_SESSIONREJECT_WITH_SANITIZED_REASON_REPORTED',
    'LMAX_R191_FAIL_MARKETDATAREQUESTREJECT_WITH_SANITIZED_REASON_REPORTED',
    'LMAX_R191_FAIL_MARKETDATA_RESPONSE_BOUNDARY',
    'LMAX_R191_FAIL_STATE_REPORTING_FIELDS_MISSING_OR_INCONSISTENT',
    'LMAX_R191_FAIL_REJECT_REASON_EXTRACTION_MISSING',
    'LMAX_R191_FAIL_RUNTIME_BOUNDARY_INCONCLUSIVE',
    'LMAX_R191_FAIL_SHUTDOWN_REVERT_INCOMPLETE',
    'LMAX_R191_FAIL_OPERATOR_APPROVAL_MISSING_OR_MISMATCHED',
    'LMAX_R191_FAIL_MARKET_HOURS_CONSTRAINT_MISSING_OR_VIOLATED',
    'LMAX_R191_FAIL_PROFILE_NOT_SELECTED',
    'LMAX_R191_FAIL_SANITIZATION_OR_SECRET_LEAK_RISK',
    'LMAX_R191_FAIL_FORBIDDEN_ACTION_RISK',
    'LMAX_R191_FAIL_API_WORKER_GATEWAY_REGRESSION',
    'LMAX_R191_FAIL_BUILD_OR_TESTS')) 'R191 classification is not allowed.'

Assert-True ($activation.classification -eq 'LMAX_R191_FAIL_SESSIONREJECT_WITH_SANITIZED_REASON_REPORTED') 'R191 classification mismatch.'
Assert-True ($r188.classification -eq 'LMAX_R188_PASS_ENHANCED_REJECT_EXTRACTION_PROPAGATION_REPAIR_IMPLEMENTED_NO_EXTERNAL') 'R188 evidence missing.'
Assert-True ($r189.classification -eq 'LMAX_R189_PASS_ENHANCED_REJECT_EXTRACTION_REPAIRED_RETRY_READINESS_NO_EXTERNAL') 'R189 readiness evidence missing.'

Assert-True ($preflight.preflightPassed -eq $true) 'R191 preflight did not pass.'
Assert-True ($preflight.approvalExactFreshMatch -eq $true) 'R191 approval was not exact/fresh.'
Assert-True ($preflight.marketHoursConfirmationConcrete -eq $true) 'R191 concrete market-hours confirmation missing.'
Assert-True ($preflight.marketHoursWindow -eq 'Tuesday, May 19, 2026 at 10:58 Europe/Paris') 'R191 market-hours window mismatch.'
Assert-True ($preflight.externalBoundaryAllowed -eq $true) 'R191 external boundary was not allowed after approved preflight.'

Assert-True ($activation.externalActivationAttempted -eq $true) 'R191 external activation was not attempted.'
Assert-True ($activation.attemptCount -eq 1) 'R191 attemptCount must be exactly one.'
Assert-True ($selected.selectedProfile -eq $profile) 'R191 selected profile evidence mismatch.'
Assert-True ($selected.profileExecuted -eq $true) 'R191 profile was not executed.'
Assert-True ($selected.selectedDiagnosticProfileCount -eq 1) 'R191 selected more than one diagnostic profile.'
...
foreach ($field in @('marketDataRejectSanitizedSubcategory', 'sessionRejectSanitizedSubcategory', 'rejectReasonExtractionSource')) {
    Assert-True ($extraction.PSObject.Properties.Name -contains $field) "Enhanced extraction field missing: $field"
    Assert-True ($response.PSObject.Properties.Name -contains $field) "Response extraction field missing: $field"
}
Assert-True ($extraction.r188PropagationRepairActive -eq $true) 'R188 propagation repair not active.'
...
Assert-True ($extraction.specificSanitizedRejectSubcategoryProduced -eq $true) 'Specific sanitized reject subcategory missing.'
Assert-True ($extraction.rejectReasonNotAvailableDistinctFromPropagationFailure -eq $true) 'RejectReasonNotAvailable/propgation failure distinction missing.'
Assert-True ($extraction.enhancedRejectExtractionMissing -eq $false) 'Enhanced reject extraction missing.'
...
Assert-True ($forbidden.externalActivationAttempted -eq $true) 'R191 forbidden audit did not record approved external attempt.'
Assert-True ($forbidden.socketOpened -eq $true) 'R191 socket boundary missing.'
Assert-True ($forbidden.tlsOpened -eq $true) 'R191 TLS boundary missing.'
Assert-True ($forbidden.fixSessionOpened -eq $true) 'R191 FIX boundary missing.'
Assert-True ($forbidden.liveMarketDataRequestSent -eq $true) 'R191 MarketDataRequest boundary missing.'
Assert-True ($forbidden.liveMarketDataResponseRead -eq $true) 'R191 MarketDataResponse boundary missing.'
...
Assert-True ($next.mustRemainNoExternal -eq $true) 'R192 must remain no-external.'
Assert-True ($next.doNotRecommendBlindLiveRetry -eq $true) 'Next phase must not recommend a blind live retry.'

$artifactText = Get-ChildItem -LiteralPath $ArtifactRoot -Filter 'phase-lmax-r191-*' -File |
    Where-Object { $_.Name -notin @('phase-lmax-r191-operator-approval.txt', 'phase-lmax-r191-expected-operator-approval.txt') } |
...
foreach ($forbiddenToken in @('35=V', '35=Y', '35=3', '281=', '371=', '372=', '373=', '58=', [string][char]1, 'password=', 'username=', 'SenderCompId', 'TargetCompId')) {
    Assert-True (-not $combinedArtifactText.Contains($forbiddenToken)) "Forbidden serialized token detected in R191 artifacts: $forbiddenToken"
}
...
Assert-True ($gate.validator -eq 'LMAX_R191_VALIDATION_PASS') 'Validator evidence missing.'

'LMAX_R191_VALIDATION_PASS'
