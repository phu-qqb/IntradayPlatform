...
$summary = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-temporary-readonly-activation-retry-summary.json")
$preflight = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-preflight-result.json")
$endpoint = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-demo-endpoint-binding-evidence.json")
$socket = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-socket-connector-evidence.json")
$tls = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-tls-boundary-evidence.json")
$credential = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-fix-credential-material-evidence.json")
$fix = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-fix-session-boundary-evidence.json")
$market = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-marketdata-request-evidence.json")
$trace = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-operational-invocation-trace.json")
$boundary = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-boundary-evidence.json")
$marketResult = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-marketdata-sanitized-result.json")
$forbidden = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-forbidden-actions-audit.json")
$apiWorker = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-api-worker-fake-gateway-audit.json")
$usdJpy = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-usdjpy-caveat-preservation.json")
$shutdown = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-shutdown-revert-evidence.json")
$next = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-next-phase-recommendation.json")
$gate = Read-Json (Join-Path $artifactRoot "phase-lmax-r85-gate-validation.json")
$r83 = Read-Json (Join-Path $artifactRoot "phase-lmax-r83-fix-credential-material-binding-summary.json")

$allowedClassifications = @(
    "LMAX_R85_PASS_TEMPORARY_READONLY_RUNTIME_ACTIVATION_SANITIZED",
    "LMAX_R85_FAIL_OPERATOR_APPROVAL_MISSING_OR_MISMATCHED",
    "LMAX_R85_FAIL_R83_SUCCESS_EVIDENCE_MISSING",
    "LMAX_R85_FAIL_DEMO_ENDPOINT_BINDING_REGRESSION",
    "LMAX_R85_FAIL_PLACEHOLDER_HOST_USED",
    "LMAX_R85_FAIL_SOCKET_CONNECTOR_BINDING_REGRESSION",
    "LMAX_R85_FAIL_TLS_PROGRESSION_BINDING_REGRESSION",
    "LMAX_R85_FAIL_FIX_PROGRESSION_BINDING_REGRESSION",
    "LMAX_R85_FAIL_FIX_CREDENTIAL_MATERIAL_BINDING_REGRESSION",
    "LMAX_R85_FAIL_REAL_SECRET_MATERIAL_NOT_ALLOWED_FOR_APPROVED_RETRY",
    "LMAX_R85_FAIL_REAL_SECRET_MATERIAL_NOT_LOADED_FOR_APPROVED_RETRY",
    "LMAX_R85_FAIL_FIX_ATTEMPT_ALLOWED_WITHOUT_TLS_SUCCESS",
    "LMAX_R85_FAIL_MARKETDATA_ATTEMPT_ALLOWED_WITHOUT_FIX_SUCCESS",
    "LMAX_R85_FAIL_MANUAL_EXECUTION_SURFACE_REGRESSION",
    "LMAX_R85_FAIL_REAL_BOUNDED_ADAPTER_BINDING_REGRESSION",
    "LMAX_R85_FAIL_NO_EXTERNAL_ADAPTER_USED_INSTEAD_OF_REAL_BOUNDARY_ADAPTER",
    "LMAX_R85_FAIL_EXECUTE_ONCE_NOT_INVOKED",
    "LMAX_R85_FAIL_PRE_EXTERNAL_APPROVAL_OR_COMPOSITION_REGRESSION",
    "LMAX_R85_FAIL_CREDENTIAL_CONFIG_MISSING",
    "LMAX_R85_FAIL_CREDENTIAL_CONFIG_NOT_APPROVED",
    "LMAX_R85_FAIL_CREDENTIAL_CONFIG_BINDING_REGRESSION",
    "LMAX_R85_FAIL_PRODUCTION_ACCOUNT_RISK",
    "LMAX_R85_FAIL_API_WORKER_GATEWAY_REGRESSION",
    "LMAX_R85_FAIL_LIVE_LAUNCHER_OR_BACKGROUND_SERVICE_RISK",
    "LMAX_R85_FAIL_FORBIDDEN_ACTION_RISK",
    "LMAX_R85_FAIL_USDJPY_CAVEAT_WEAKENED",
    "LMAX_R85_FAIL_PREFLIGHT_ABORTED",
    "LMAX_R85_FAIL_TCP_SOCKET_BOUNDARY",
    "LMAX_R85_FAIL_TLS_BOUNDARY",
    "LMAX_R85_FAIL_FIX_LOGON_OR_SESSION_BOUNDARY",
    "LMAX_R85_FAIL_MARKETDATA_REQUEST_BOUNDARY",
    "LMAX_R85_FAIL_MARKETDATA_RESPONSE_BOUNDARY",
    "LMAX_R85_FAIL_RUNTIME_BOUNDARY_INCONCLUSIVE",
    "LMAX_R85_FAIL_SHUTDOWN_REVERT_INCOMPLETE",
    "LMAX_R85_FAIL_SANITIZATION_OR_SECRET_LEAK_RISK",
    "LMAX_R85_FAIL_BUILD_OR_TESTS"
)

Assert-True ($allowedClassifications -contains $summary.classification) "R85 classification is absent or not allowed."
Assert-True ($summary.classification -eq "LMAX_R85_FAIL_FIX_LOGON_OR_SESSION_BOUNDARY") "R85 classification mismatch."
Assert-True ($r83.classification -eq "LMAX_R83_PASS_FIX_CREDENTIAL_MATERIAL_BINDING_READY_NO_EXTERNAL_ACTIVATION") "R83 success evidence missing."
...
Assert-True ($summary.retryPhaseReservationPassed -eq $true) "LMAX-R85 retry phase reservation did not pass."
...
Assert-True ($summary.inMemoryOnlyFixCredentialMaterialAllowed -eq $true) "In-memory FIX material was not allowed."
Assert-True ($summary.inMemoryOnlyFixCredentialMaterialLoadedSanitizedBoolean -eq $true) "In-memory FIX material was not loaded as sanitized boolean evidence."
...
Assert-True ($summary.fixBoundaryResultCategory -eq "FixFrameWriteNotImplemented") "Unexpected FIX boundary category."
...
Assert-False $summary.rawCredentialsFixFieldsMessagesPrintedStoredSerialized "Raw credential/FIX values were printed/stored/serialized."
...
Assert-True ($preflight.r83SuccessEvidencePresent -eq $true) "R83 success evidence missing."
...
Assert-True ($credential.realSecretMaterialAllowedNow -eq $true) "RealSecretMaterialAllowedNow not true."
Assert-True ($credential.realSecretMaterialLoadedInMemory -eq $true) "RealSecretMaterialLoaded not true."
Assert-True ($credential.realSecretMaterialLoadedSanitizedBooleanOnly -eq $true) "Material loaded evidence is not boolean-only."
Assert-False $credential.credentialValuesReturned "Credential values returned."
Assert-False $credential.sensitiveMaterialReturned "Sensitive material returned."
Assert-False $credential.sensitiveMaterialPrinted "Sensitive material printed."
Assert-False $credential.sensitiveMaterialStored "Sensitive material stored."
Assert-False $credential.sensitiveMaterialSerialized "Sensitive material serialized."
Assert-False $credential.rawSecretSerialized "Raw secret serialized."
Assert-False $credential.rawFixSerialized "Raw FIX serialized."
Assert-False $credential.apiWorkerReachable "Credential material API/Worker reachable."
...
Assert-True ($fix.fixBoundaryResultCategory -eq "FixFrameWriteNotImplemented") "FIX category mismatch."
Assert-True ($fix.fixAttemptedOnlyAfterTlsSucceeded -eq $true) "FIX was allowed without TLS success."
Assert-True ($fix.credentialMaterialUnavailableCleared -eq $true) "Credential material blocker not cleared."
...
Assert-False $fix.rawFixMessagesSerialized "Raw FIX messages serialized."
...
Assert-False $market.marketDataAttemptAllowedWithoutFixSuccess "MarketDataRequest allowed without FIX success."
...
Assert-True ($boundary.boundaryStatuses.credentialConfig -eq "Succeeded") "Credential/config boundary mismatch."
...
Assert-False $apiWorker.fixCredentialMaterialReachableFromApiWorkerDefaultStartup "Credential material reachable from API/Worker/default startup."
...
Assert-True ($next.nextRecommendedPhase -eq "LMAX-R86") "Next phase recommendation missing or incorrect."
Assert-True ($next.nextRecommendedTitle -eq "FIX Session Boundary Root-Cause Pack") "Next phase title mismatch."

$artifactText = Get-ChildItem $artifactRoot -Filter "phase-lmax-r85-*" -File |
    Where-Object { $_.Name -ne "phase-lmax-r85-operator-approval-note.md" } |
...
foreach ($pattern in $forbiddenPatterns) {
    if ($joined -match [regex]::Escape($pattern)) {
        throw "Forbidden sensitive artifact pattern found in R85 artifacts: $pattern"
    }
}

Write-Output "LMAX_R85_VALIDATION_PASS"
