...
using Retirement = QQ.Production.Intraday.Application.LmaxDemoBboNoSendRetirement;
...
internal static class BboNoSendRetirementCommand
{
    ...
    internal static async Task<int> Run(string[] args)
    {
        ...
        await using (var update = connection.CreateCommand())
        {
            ...
            update.CommandText = """
                ...
                WHERE Id='dd4194b6-01a0-4561-9708-a801b3973bd6'
                  AND ParentOrderId='20c4d977-05e4-46b5-8dc3-34c887ed26a4' AND Status=@pending;
                IF @@ROWCOUNT<>1 THROW 51000, 'EXACT_CHILD_EXPIRY_FAILED', 1;
                UPDATE ParentOrders SET Status=@expired
                WHERE Id='20c4d977-05e4-46b5-8dc3-34c887ed26a4'
                  AND TradeIntentId='45b4ff1a-047f-4bed-a552-79b6d53c420c' AND Status=@created;
        ...
        Console.WriteLine(JsonSerializer.Serialize(new { marker = "EXACT_BBO_NO_SEND_RETIRED", certificate,
        ...
        SqlTransaction transaction, DateTimeOffset observedAt, DateTimeOffset dispositionAt)
    {
        ...
        command.CommandText = """
            ...
            WHERE f.Name='QQ Intraday Fund' AND f.IsEnabled=1 AND b.AccountCode='LMAX_DEMO_LOCAL'
              ...
              AND m.AsOfUtc='2026-09-17T16:15:00+00:00' AND m.EffectiveAtUtc='2026-09-17T16:30:00+00:00' AND m.NavUsd=1000000
              AND t.Id='45b4ff1a-047f-4bed-a552-79b6d53c420c'
              AND p.Id='20c4d977-05e4-46b5-8dc3-34c887ed26a4'
              AND c.Id='dd4194b6-01a0-4561-9708-a801b3973bd6'
            """;
        ...
        if (reader.GetDecimal(17) != .1m || reader.GetDecimal(18) != .1m)
