...
public static class LmaxMarketDataCaptureOnlyPreflightCommand
{
    ...
    public static async Task<int> RunAsync(string[] args,CancellationToken cancellationToken=default)
    {
        ...
        var config=JsonSerializer.Deserialize<LmaxMarketDataOnlyPreflightConfig>(await File.ReadAllTextAsync(configPath,cancellationToken),CanonicalRecorderV2Constants.JsonOptions)??throw new InvalidOperationException("config_deserialize_failed");
        if(string.Equals(command,"preflight",StringComparison.OrdinalIgnoreCase))
        {
            ...
            var report=LmaxMarketDataOnlyPreflight.Validate(config,parsed.ContainsKey("network-disabled"),parsed.ContainsKey("no-order-entry"),parsed.ContainsKey("no-account-api"),parsed.ContainsKey("no-db"));
        ...
        var preflight=LmaxMarketDataOnlyPreflight.Validate(config,networkDisabled:true,noOrderEntry:true,noAccountApi:true,noDb:true);
    ...
    private static string FindRepoRoot(){var dir=new DirectoryInfo(AppContext.BaseDirectory);while(dir is not null){if(File.Exists(Path.Combine(dir.FullName,"QQ.Production.Intraday.sln")))return dir.FullName;dir=dir.Parent;}throw new DirectoryNotFoundException("repo_root_not_found");}
