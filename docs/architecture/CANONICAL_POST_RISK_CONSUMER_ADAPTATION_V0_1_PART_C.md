...
## Conservative consumed-block retry semantics

The immutable canonical receipt is recorded before retained-state and release evaluation. Therefore an identical adapter input identity, revision, and fingerprint that is locally blocked is conservatively consumed: its same-revision replay is returned as a duplicate and is not implicitly re-evaluated or re-armed. A valid higher canonical correction/revision remains distinct. Durable retry/re-arm policy is an explicit later operational decision; it is not invented by this adapter.
