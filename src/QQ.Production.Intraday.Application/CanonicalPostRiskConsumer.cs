...
public static class CanonicalPostRiskInputParser
{
    ...
    public static CanonicalPostRiskInput Parse(string json)
    {
        ...
        var expected = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(Material(root)))).ToLowerInvariant();
    ...
    private static string Material(JsonElement root)
    {
        ...
        writer.WritePropertyName("riskInputSnapshot"); writer.WriteStartObject(); var ri = root.GetProperty("riskInputSnapshot"); foreach (var name in new[] { "snapshotId", "effectiveAt", "recordedAt", "provenance" }) S(writer, ri, name); writer.WriteEndObject();
        foreach (var name in new[] { "riskEvaluatedAt", "riskRecordedAt" }) S(writer, root, name);
        ...
        foreach (var name in new[] { "effectiveAt", "recordedAt", "knowledgeCutoff", "provenance", "decision" }) S(writer, root, name); writer.WriteEndObject(); writer.Flush(); return Encoding.UTF8.GetString(stream.ToArray());
    }
    ...
    private static void S(Utf8JsonWriter w, JsonElement x, string n) => w.WriteString(n, x.GetProperty(n).GetString()); private static void N(Utf8JsonWriter w, JsonElement x, string n) => w.WriteNumber(n, x.GetProperty(n).GetInt64()); private static void Fail(string m) => throw new ArgumentException(m, "json");
}
...
public sealed record RetainedExecutionContext(decimal NavUsd, int FrequencyMinutes, TargetQuantityMode TargetQuantityMode, BrokerAccountId BrokerAccountId, string RoutingContext, string ContextId, long Revision, DateTimeOffset EffectiveFromUtc, DateTimeOffset? EffectiveToUtc, string Provenance);
...
public sealed class InMemoryCanonicalExecutionContextResolver(IEnumerable<MandateFundMapping> mandateFunds, IEnumerable<InstrumentExecutionMapping> instruments, IEnumerable<RetainedExecutionContext> contexts) : ICanonicalExecutionContextResolver
{
    public ResolvedCanonicalExecutionContext Resolve(CanonicalPostRiskInput input, DateTimeOffset asOfUtc)
    {
        ...
        var context = One(contexts.Where(x => Active(x.EffectiveFromUtc, x.EffectiveToUtc, asOfUtc)), "execution context");
